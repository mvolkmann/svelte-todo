<script>
  ...
  const onArchiveCompleted = () => todos = todos.filter(t => !t.done);

  const onDeleteTodo = todoId => todos = todos.filter(t => t.id !== todoId);
...
<div>
  ...
  <div>
    ...
    <button on:click={onArchiveCompleted}>Archive Completed</button>
  </div>
  <br />
  <form>
    <input
      ...
      bind:value={todoText}
    />
    <button disabled={!todoText} on:click={onAddTodo}>
      Add
    </button>
  </form>
  <ul class="unstyled">
